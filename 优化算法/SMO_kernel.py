...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 18 14:25:20 2023
...
class SSGD:
    def __init__(self,iteration,use_linear_optim = True):
    ...
    def fit(self,X,Y):
        ...
        return w[0:-1],w[-1]
    def sub_grid(self,X,Y,w):
        result = []
        
        return result
        
    def grad_l_i(self,i):
        if self.Y[i]*self.f(self.X[i]) < 1:
            return -self.Y[i]*self.kernel(self.X[i])
        else:
            return 0.
...
if __name__ == '__main__':
    ...
    #print('test accuracy:',model.get_accuracy(X_test,y_test))    #alphas, b = platt_smo(X_train,y_train,0.8,40)
